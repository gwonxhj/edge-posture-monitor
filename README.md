...
UART 기반 실시간 센서 스트리밍 -> 자세 feature 추출 -> rule based 기반 자세 판별 -> 세션 리포트 생성까지 수행하는 End-to-End 자세 분석 시스템입니다.
...
Raspberry Pi는 UART 바이너리 패킷 수신, 센서 보정, feature 추출, rule-based 자세 판별, 상태 관리, 리포트 생성, SQLite 저장을 담당합니다.

본 프로젝트는 단순 센서 수집 수준이 아니라,
STM32 기반 센서 노드와 Raspberry Pi 기반 엣지 런타임을 분리하여
실시간 수신, calibration baseline 생성, 센서 보정, 자세 분석, 세션 리포트 저장, 앱 실시간 전송까지 포함하는 임베디드-엣지 통합 시스템으로 설계되었습니다.
...
- **Enhanced Session Report**
  ...
  - QWen 3.5 0.8B 4bit quantized GGUF + llama-cpp 기반 live LLM report 생성
...
### 1. Mock Validation Mode
- Fake STM32 기반 end-to-end 테스트
- UART / 상태머신 / 리포트 생성 / DB 저장 흐름 사전 검증
- 실제 하드웨어 없이 backend / app / report 구조를 먼저 안정화

### 2. Real Hardware Integration Mode
- 실제 STM32 센서 노드와 Raspberry Pi 실연동
- UART handshake, CHK_SIT / SIT / CAL / GO timing 안정화
- 실측 데이터 기반 sensor mapping / feature / threshold 보정 진행
...
Raspberry Pi는 파싱, 센서 보정, feature 추출, rule-based 자세 판별, 점수 계산, 리포트 생성을 담당하는 Edge Runtime으로 구성된다.
...
또한 LLM 실패 시에도 동일 schema를 유지하도록 rule-based fallback 경로를 함께 구성했습니다.
...
- baselines  
 : 사용자별 calibration baseline 저장

- sessions  
 : 측정 세션 메타데이터 및 요약 정보 저장
