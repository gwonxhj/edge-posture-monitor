...
UART 기반 실시간 센서 스트리밍, 자세 특징 추출, 규칙 기반 자세 판별, 세션 리포트 생성을 수행하는 End-to-End 자세 분석 시스템입니다.
...
Raspberry Pi는 UART 바이너리 패킷 수신, 센서 보정, 특징 추출, 규칙 기반 자세 판별, 상태 관리, 리포트 생성, SQLite 저장을 담당합니다.
...
Raspberry Pi는 파싱, 센서 보정, 특징 추출, 규칙 기반 자세 판별, 점수 계산, 리포트 생성을 담당하는 Edge Runtime으로 구성된다.
...
시스템은 규칙 기반으로 판정된 posture를 기준으로 자세 점수를 계산합니다.
...
또한 LLM 실패 시에도 동일한 출력 구조를 유지할 수 있도록 규칙 기반 대체 생성 경로를 함께 구성했습니다.
...
- 측정 종료 후 세션 요약, 분 단위 요약, 해석형 리포트, 일 단위 요약 저장
...
- daily_reports
 : 동일 날짜 내 여러 세션 결과를 사용자 기준으로 누적 집계한 일 단위 자세 요약 저장
...
│   ├── config              # 환경 설정
│   ├── core                # 특징 추출 / posture flags / score / sensor factor
